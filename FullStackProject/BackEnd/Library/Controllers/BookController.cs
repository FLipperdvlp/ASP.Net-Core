...
{
    public ActionResult<List<ActionResult<Book>>> GetAllBooks()
    {
    ...
    [HttpPost]
    public ActionResult<Book> CreateBook([FromBody] CreateBookRequestModel model)
    {
    ...
    [HttpPut("{bookId:int}")]
    public ActionResult<Book> UpdateBook(int bookId, [FromBody] UpdateBookRequestModel model)
    {
        var book = dbContext.Books.Find(bookId);
        if(book is null) return NotFound();
